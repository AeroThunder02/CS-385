/*******************************************************************************
 * Name        : rbtree.h
 * Author      : Andrew Bannon | Maciej Kowalczyk
 * Version     : 1.0
 * Date        : 
...
public:
    ...
    void insert(const iterator &it, const std::pair<K, V> &key_value) {
        ...
        //We need to check if the key already exists, we can use the find(key) function to do this
        //If there exists a dupelicate, throw an exception
        std::stringstream duplicate_msg;
        duplicate_msg << "Attempt to insert duplicate key '" << key << "'.";
        iterator check = find(key);
        if((check.node_ptr != nullptr) && (check.node_ptr->key() == key)){
        	throw tree_exception(duplicate_msg.str());
        }
...
private:
    ...
    void delete_tree(Node<K, V> *n) {
    	if(n==nullptr || n == NULL){
    		...
    		Node<K, V> *leftTree = n->left; //make new left subtree
    		Node<K, V> *rightTree = n->right; //make new right subtree
    		delete	n; //delete root
    		delete_tree(leftTree); //delete left
    		delete_tree(rightTree); //delete right
    	}
    }
    ...
    int height(Node<K, V> *node) const {
        ...
        return std::max((left_height + 1), (right_height + 1));
    }
    ...
    int diameter(Node<K, V> *node) const {
        ...
        int root_diameter = height(node->left) + height(node->right) + 1;
        int left_diameter = diameter(node->left);
        int right_diameter = diameter(node->right);
        return std::max((root_diameter), std::max(left_diameter, right_diameter));
    }
